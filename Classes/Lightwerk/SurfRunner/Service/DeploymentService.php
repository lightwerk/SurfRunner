...
class DeploymentService
{
    ...
    public function deployByIdentifier($identifier, LoggerInterface $logger, $dryRun)
    {
        ...
        $surfCaptainDeployment = $this->persistenceManager->getObjectByIdentifier(
            $identifier,
            'Lightwerk\SurfCaptain\Domain\Model\Deployment'
        );
        if ($surfCaptainDeployment instanceof SurfCaptainDeployment === false) {
            throw new NoAvailableDeploymentException(
                'deployment with identifier ' . $identifier . ' not found',
                1428685495
            );
        }
        if (!$dryRun) {
        ...
        $logger->addBackend(
            new DatabaseBackend(
                ['deployment' => $surfCaptainDeployment, 'severityThreshold' => LOG_DEBUG]
            )
        );
        try {
            ...
            throw new NoAvailableDeploymentException(
                'cannot create deployment with DeploymentFactoryException ' . $e->getMessage() . ' - ' . $e->getCode(),
                1428769117
            );
        }
        ...
        if (!$dryRun) {
            ...
            try {
                ...
                $this->setStatusAfterDeployment($surfCaptainDeployment, Deployment::STATUS_FAILED);
                throw new NoAvailableDeploymentException(
                    'cannot deploy with InvalidConfigurationException' . $e->getMessage() . ' - ' . $e->getCode(),
                    1428769119
                );
            }
    ...
    public function deployWaitingFromQueue(LoggerInterface $logger, $dryRun)
    {
        ...
        if ($this->deploymentRepository->countByRepositoryUrlAndStatus(
            $surfCaptainDeployment->getRepositoryUrl(),
            SurfCaptainDeployment::STATUS_RUNNING
        ) > 0) {
